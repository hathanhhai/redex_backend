pipeline {
    ...
    stages {
        ...
        stage('Docker Build and Deploy Production'){
            steps{
                 sh 'docker rm -f redex_backend &> /dev/null'
                 sh 'docker compose -f docker-compose.yml -p redex_backend up --build -d --force-recreate'
                 sh 'docker exec  redex_backend sh -c "chmod 777 -R storage/ && chmod 777 -R public/"'
